...
import { descargarRecibo } from '../core/recibo.js';
...
function showReciboConfirm(p) {
  ...
  $('reciboBody').innerHTML = `
    ...
    <div class="btn-row" style="margin-top:14px">
      <button class="btn btn-gold btn-block" onclick="App.descargarUltimoRecibo()">⬇ Descargar recibo PDF</button>
    </div>`;
  card.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

// Descargar el recibo del pago recién registrado
export function descargarUltimoRecibo() {
  if (ultimoPago) descargarRecibo(ultimoPago);
}

// Descargar el recibo de cualquier pago (usado desde el Historial)
export function descargarReciboPago(id) {
  const p = state.pagos.find(x => x.id === id);
  if (p) descargarRecibo(p);
}
