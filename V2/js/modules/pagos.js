...
import { descargarReciboPDF, descargarReciboJPG, compartirReciboWhatsApp } from '../core/recibo.js';
...
function showReciboConfirm(p) {
  ...
  $('reciboBody').innerHTML = `
    ...
    <button class="btn btn-wa btn-block" style="margin-top:14px" onclick="App.compartirUltimoWhatsApp()">📲 Enviar por WhatsApp</button>
    <div class="btn-row" style="margin-top:10px">
      <button class="btn btn-gold" style="flex:1" onclick="App.descargarUltimoReciboPDF()">📄 PDF</button>
      <button class="btn btn-primary" style="flex:1" onclick="App.descargarUltimoReciboJPG()">🖼 JPG</button>
    </div>`;
  card.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

// Recibo del pago recién registrado
export function descargarUltimoReciboPDF() { if (ultimoPago) descargarReciboPDF(ultimoPago); }
export function descargarUltimoReciboJPG() { if (ultimoPago) descargarReciboJPG(ultimoPago); }
export function compartirUltimoWhatsApp() { if (ultimoPago) compartirReciboWhatsApp(ultimoPago); }

// Recibo de cualquier pago (usado desde el Historial)
export function descargarReciboPagoPDF(id) {
  const p = state.pagos.find(x => x.id === id);
  if (p) descargarReciboPDF(p);
}
export function descargarReciboPagoJPG(id) {
  const p = state.pagos.find(x => x.id === id);
  if (p) descargarReciboJPG(p);
}
export function compartirPagoWhatsApp(id) {
  const p = state.pagos.find(x => x.id === id);
  if (p) compartirReciboWhatsApp(p);
}
